...
export default async function ItemsPage({ searchParams }: PageProps) {
  ...
  const locations = [...new Set((locRows ?? []).map((r) => r.location as string).filter(Boolean))].sort();
    ...
    <div>
      ...
      {!items || items.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-24 text-center">
          ...
          {isFiltered ? (
            <>
              <p className="text-zinc-600 font-medium mb-1">No items match your search</p>
              <p className="text-zinc-400 text-sm">Try a different name or location</p>
            </>
          ) : (
            <>
